...
if st.button("🗑️ Delete Chat"):
    ...
    # Delete current chat file
    if os.path.exists(chat_path):
        os.remove(chat_path)
        deleted = True

    # Delete all history files containing both users' names
    for file in os.listdir(HISTORY_DIR):
        if user in file or partner in file:
            os.remove(history_file(file))
            deleted = True

    if deleted:
        st.success("Chat and related history deleted!")
        st.rerun()
    else:
