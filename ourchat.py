...
# --- STYLING ---
st.markdown("""
    <link href="https://fonts.googleapis.com/css2?family=Dancing+Script:wght@600&family=Ubuntu:wght@400;500&display=swap" rel="stylesheet">
    <style>
    .chat-bubble {
...
def render_message_bubble(sender, message, timestamp, current_user, is_read=False):
    ...
    # ✅ Blue Tick Logic
    ticks = "<span style='color:#34B7F1;'>✔✔</span>" if (sender == current_user and is_read) else (
        "<span style='color:gray;'>✔</span>" if sender == current_user else ""
    )
    ...
    <div style="display:flex; justify-content:{align}; margin:6px 0;">
        <div class="chat-bubble" style="
            ...
            text-align:center;
        ">
            <b style="font-family:'Dancing Script', cursive; font-size:18px; color:{text_color}; font-weight:600; font-style:italic;">
                {display_name}
            </b><br>
            <span style="font-weight:400;">{message}</span>
            <div style="font-size:12px; color:gray; text-align:right;">🕒 {timestamp} {ticks}</div>
        </div>
    </div>
    """
...
if st.sidebar.button("📝 New Chat"):
    save_chat_to_history(load_messages(user, partner))
    save_messages(user, partner, [])
    st.success("Started a new chat!")
    st.rerun()

# HISTORY DROPDOWN
# HISTORY DROPDOWN WITH DELETE OPTION
if history_files:
    ...
    # 🗑️ Delete Selected History Button
    if st.sidebar.button("🗑️ Delete Selected History"):
        try:
            os.remove(history_file(selected))
            st.sidebar.success(f"Deleted: {selected}")
            st.rerun()
        except:
            st.sidebar.error("⚠️ Failed to delete history.")
